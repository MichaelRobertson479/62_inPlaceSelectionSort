...
public class OrderedList_inArraySlots
    ...
    public OrderedList_inArraySlots ( ArrayList<Integer> unordered) {
        ...
        for (int start = 0; start < list_iAS.size(); start++) {
          int champIndex = champIndex(start);
          int storage = list_iAS.get(start);
          list_iAS.set(start,list_iAS.get(champIndex));
          list_iAS.set(champIndex,storage);

        System.out.println(list_iAS + System.lineSeparator());
      }
    }
     ...
     */
     ...
     private int champIndex(int start) {
       ...
       for (int i = start; i < list_iAS.size(); i++) {
         if (list_iAS.get(i) != null && list_iAS.get(i) < champ) {
